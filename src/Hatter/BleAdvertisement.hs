...
-- address and RSSI. Service data is keyed by the full 128-bit UUID in
-- lowercase text form (16- and 32-bit UUIDs are expanded with the
-- Bluetooth base UUID, matching the textual convention of
-- "Hatter.Ble"); manufacturer data is keyed by the 16-bit company
-- identifier. Entries keep their advertisement order.
data BleAdvertisement = BleAdvertisement
  { advServiceData :: [(Text, ByteString)]
...
addAdStructure adType payload adv = if
  | adType == 0x16 -> addServiceData 2 payload adv
  | adType == 0x20 -> addServiceData 4 payload adv
  | adType == 0x21 -> addServiceData 16 payload adv
  | adType == 0xFF -> addManufacturerData payload adv
  | otherwise -> adv
...
addServiceData uuidWidth payload adv =
  if BS.length payload < uuidWidth
    then adv
    else
      let (uuidBytes, dataBytes) = BS.splitAt uuidWidth payload
      in adv { advServiceData =
                 (uuidTextFromLittleEndian uuidBytes, dataBytes) : advServiceData adv }
...
addManufacturerData payload adv =
  if BS.length payload < 2
    then adv
      ...
      in adv { advManufacturerData =
                 (ManufacturerId companyId, dataBytes) : advManufacturerData adv }
...
serviceDataForUuid uuid adv =
  lookup (Text.toLower uuid) (advServiceData adv)
...
-- air) as full lowercase 128-bit text.
uuidTextFromLittleEndian :: ByteString -> Text
uuidTextFromLittleEndian uuidBytes =
